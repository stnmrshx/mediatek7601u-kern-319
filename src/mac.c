...
static void
mt76_mac_fill_tx_status(struct mt7601u_dev *dev, struct ieee80211_tx_info *info, struct mt76_tx_status *st)
{
...
void mt76_mac_wcid_set_rate(struct mt7601u_dev *dev, struct mt76_wcid *wcid, const struct ieee80211_tx_rate *rate)
{
	unsigned long flags;
...
{
	struct mt7601u_dev *dev = container_of(work, struct mt7601u_dev, mac_work.work);
	struct {
		u32 addr_base;
		u32 span;
		u64 *stat_base;
...
void mt7601u_mac_set_ampdu_factor(struct mt7601u_dev *dev)
{
	...
	rcu_read_unlock();
	mt7601u_wr(dev, MT_MAX_LEN_CFG, 0xa0fff | MT76_SET(MT_MAX_LEN_CFG_AMPDU, min_factor));
}
...
static int mt7601u_rx_is_our_beacon(struct mt7601u_dev *dev, u8 *data)
{
	...
	return ieee80211_is_beacon(hdr->frame_control) && ether_addr_equal(hdr->addr2, dev->ap_bssid);
}
...
int mt76_mac_wcid_set_key(struct mt7601u_dev *dev, u8 idx, struct ieee80211_key_conf *key)
{
	...
	val |= MT76_SET(MT_WCID_ATTR_PKEY_MODE, cipher & 7) | MT76_SET(MT_WCID_ATTR_PKEY_MODE_EXT, cipher >> 3);
	val &= ~MT_WCID_ATTR_PAIRWISE;
	val |= MT_WCID_ATTR_PAIRWISE * !!(key && key->flags & IEEE80211_KEY_FLAG_PAIRWISE);
...
int mt76_mac_shared_key_setup(struct mt7601u_dev *dev, u8 vif_idx, u8 key_idx, struct ieee80211_key_conf *key)
{
	...
	mt7601u_wr_copy(dev, MT_SKEY(vif_idx, key_idx), key_data, sizeof(key_data));
