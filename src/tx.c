...
mt7601u_push_txwi(struct mt7601u_dev *dev, struct sk_buff *skb, struct ieee80211_sta *sta, struct mt76_wcid *wcid,
		  int pkt_len)
{
	struct ieee80211_tx_info *info = IEEE80211_SKB_CB(skb);
	...
	if ((info->flags & IEEE80211_TX_CTL_AMPDU) && sta) {
		...
		txwi->flags = cpu_to_le16(MT_TXWI_FLAGS_AMPDU | MT76_SET(MT_TXWI_FLAGS_MPDU_DENSITY, sta->ht_cap.ampdu_density));
		if (info->flags & IEEE80211_TX_CTL_RATE_CTRL_PROBE)
			txwi->flags = 0;
	}
...
{
	struct mt7601u_dev *dev = container_of(work, struct mt7601u_dev, stat_work.work);
	...
	if (cleaned)
		...
		queue_delayed_work(dev->stat_wq, &dev->stat_work, msecs_to_jiffies(20));
	else
		clear_bit(MT7601U_STATE_READING_STATS, &dev->state);
	spin_unlock_irqrestore(&dev->tx_lock, flags);
}

int mt7601u_conf_tx(struct ieee80211_hw *hw, struct ieee80211_vif *vif, u16 queue, const struct ieee80211_tx_queue_params *params)
{
	struct mt7601u_dev *dev = hw->priv;
