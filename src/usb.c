...
int mt7601u_usb_submit_buf(struct mt7601u_dev *dev, int dir, int ep_idx, struct mt7601u_dma_buf *buf, gfp_t gfp, usb_complete_t complete_fn, void *context)
{
	struct usb_device *usb_dev = mt7601u_to_usb_dev(dev);
	unsigned pipe;
	int ret;
	...
	usb_fill_bulk_urb(buf->urb, usb_dev, pipe, buf->buf, buf->len, complete_fn, context);
...
static int __mt7601u_vendor_request(struct mt7601u_dev *dev, const u8 req,
			 ...
			 void *buf, const size_t buflen)
{
	...
	for (i = 0; i < MT_VEND_REQ_MAX_RETRY; i++) {
		ret = usb_control_msg(usb_dev, pipe, req, req_type, val, offset, buf, buflen, MT_VEND_REQ_TOUT_MS);
		trace_mt_vend_req(dev, pipe, req, req_type, val, offset, buf, buflen, ret);
...
int mt7601u_vendor_request(struct mt7601u_dev *dev, const u8 req, const u8 direction, const u16 val, const u16 offset, void *buf, const size_t buflen)
{
	int ret;
	mutex_lock(&dev->vendor_req_mutex);
	ret = __mt7601u_vendor_request(dev, req, direction, val, offset, buf, buflen);
	if (ret == -ENODEV)
...
u32 mt7601u_rr(struct mt7601u_dev *dev, u32 offset)
{
	...
	ret = mt7601u_vendor_request(dev, MT_VEND_MULTI_READ, USB_DIR_IN, 0, offset, &reg, sizeof(reg));
	val = le32_to_cpu(reg);
...
int mt7601u_vendor_single_wr(struct mt7601u_dev *dev, const u8 req, const u16 offset, const u32 val)
{
	...
	ret = mt7601u_vendor_request(dev, req, USB_DIR_OUT, val & 0xffff, offset, NULL, 0);
	if (ret)
		return ret;
	return mt7601u_vendor_request(dev, req, USB_DIR_OUT, val >> 16, offset + 2, NULL, 0);
}
...
void mt7601u_wr_copy(struct mt7601u_dev *dev, u32 offset, const void *data, int len)
{
...
static int mt7601u_assign_pipes(struct usb_interface *usb_intf, struct mt7601u_dev *dev)
{
	...
	for (i = 0; i < intf_desc->desc.bNumEndpoints; i++) {
		...
		if (usb_endpoint_is_bulk_in(ep_desc) && ep_i++ < __MT_EP_IN_MAX) {
			dev->in_eps[ep_i - 1] = usb_endpoint_num(ep_desc);
			dev->in_max_packet = usb_endpoint_maxp(ep_desc);
			dev->in_eps[ep_i - 1] |= USB_DIR_IN;
		} else if (usb_endpoint_is_bulk_out(ep_desc) && ep_o++ < __MT_EP_OUT_MAX) {
...
static int mt7601u_probe(struct usb_interface *usb_intf, const struct usb_device_id *id)
{
