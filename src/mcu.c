...
static inline void mt7601u_dma_skb_wrap_cmd(struct sk_buff *skb, u8 seq, enum mcu_cmd cmd)
{
	WARN_ON(mt7601u_dma_skb_wrap(skb, CPU_TX_PORT, DMA_COMMAND, MT76_SET(MT_TXD_CMD_INFO_SEQ, seq) | MT76_SET(MT_TXD_CMD_INFO_TYPE, cmd)));
}

static inline void trace_mt_mcu_msg_send_cs(struct mt7601u_dev *dev, struct sk_buff *skb, bool need_resp)
{
	u32 i, csum = 0;
...
static int mt7601u_mcu_wait_resp(struct mt7601u_dev *dev, u8 seq)
{
	...
	while (i--) {
		...
		ret = mt7601u_usb_submit_buf(dev, USB_DIR_IN, MT_EP_IN_CMD_RESP, &dev->mcu.resp, GFP_KERNEL, mt7601u_complete_urb, &dev->mcu.resp_cmpl);
		if (ret)
			return ret;
...
static int mt7601u_mcu_msg_send(struct mt7601u_dev *dev, struct sk_buff *skb, enum mcu_cmd cmd, bool wait_resp)
{
	...
	unsigned cmd_pipe = usb_sndbulkpipe(usb_dev, dev->out_eps[MT_EP_OUT_INBAND_CMD]);
	int sent, ret;
	u8 seq = 0;
...
int mt7601u_mcu_tssi_read_kick(struct mt7601u_dev *dev, int use_hvga)
{
	...
	ret = mt7601u_mcu_function_select(dev, ATOMIC_TSSI_SETTING, use_hvga);
	if (ret) {
...
int mt7601u_write_reg_pairs(struct mt7601u_dev *dev, u32 base, const struct mt76_reg_pair *data, int n)
{
...
int mt7601u_burst_write_regs(struct mt7601u_dev *dev, u32 offset,const u32 *data, int n)
{
	...
	return mt7601u_burst_write_regs(dev, offset + cnt * 4, data + cnt, n - cnt);
}
...
static int __mt7601u_dma_fw(struct mt7601u_dev *dev, const struct mt7601u_dma_buf *dma_buf, const void *data, u32 len, u32 dst_addr)
{
	...
	ret = mt7601u_vendor_single_wr(dev, MT_VEND_WRITE_FCE, MT_FCE_DMA_ADDR, dst_addr);
	if (ret)
		return ret;
	len = roundup(len, 4);
	ret = mt7601u_vendor_single_wr(dev, MT_VEND_WRITE_FCE, MT_FCE_DMA_LEN, len << 16);
	if (ret)
		return ret;

	buf.len = MT_DMA_HDR_LEN + len + 4;
	ret = mt7601u_usb_submit_buf(dev, USB_DIR_OUT, MT_EP_OUT_INBAND_CMD, &buf, GFP_KERNEL, mt7601u_complete_urb, &cmpl);
	if (ret)
		return ret;
...
static int
mt7601u_dma_fw(struct mt7601u_dev *dev, struct mt7601u_dma_buf *dma_buf, const void *data, int len, u32 dst_addr)
{
	int n, ret;
...
static int mt7601u_upload_firmware(struct mt7601u_dev *dev, const struct mt76_fw *fw)
{
	...
	ret = mt7601u_dma_fw(dev, &dma_buf, fw->ilm + ilm_len, dlm_len, MT_MCU_DLM_OFFSET);
	if (ret)
		goto error;

	ret = mt7601u_vendor_request(dev, MT_VEND_DEV_MODE, USB_DIR_OUT, 0x12, 0, ivb, sizeof(fw->ivb));
	if (ret < 0)
		goto error;
	ret = 0;
...
static int mt7601u_load_firmware(struct mt7601u_dev *dev)
{
	...
	mt7601u_wr(dev, MT_USB_DMA_CFG, (MT_USB_DMA_CFG_RX_BULK_EN | MT_USB_DMA_CFG_TX_BULK_EN));
