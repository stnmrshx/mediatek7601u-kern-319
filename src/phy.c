...
#include "../include/initvals_phy.h"
...
static int mt7601u_rf_rr(struct mt7601u_dev *dev, u8 bank, u8 offset)
{
	...
	mt7601u_wr(dev, MT_RF_CSR_CFG, MT76_SET(MT_RF_CSR_CFG_REG_BANK, bank) |
				       MT76_SET(MT_RF_CSR_CFG_REG_ID, offset) | MT_RF_CSR_CFG_KICK);

	if (!mt76_poll(dev, MT_RF_CSR_CFG, MT_RF_CSR_CFG_KICK, 0, 100))
		goto out;

	val = mt7601u_rr(dev, MT_RF_CSR_CFG);
	if (MT76_GET(MT_RF_CSR_CFG_REG_ID, val) == offset && MT76_GET(MT_RF_CSR_CFG_REG_BANK, val) == bank) {
...
static int mt7601u_bbp_temp(struct mt7601u_dev *dev, int mode, const char *name)
{
	...
	ret = mt7601u_write_reg_pairs(dev, MT_MCU_MEMMAP_BBP, t[2].regs, t[2].n);
	if (ret)
		return ret;

	return mt7601u_write_reg_pairs(dev, MT_MCU_MEMMAP_BBP, t[dev->bw].regs, t[dev->bw].n);
}
...
#define FREQ_PLAN_REGS	4
	...
	ret = mt7601u_write_reg_pairs(dev, MT_MCU_MEMMAP_RF, channel_freq_plan, FREQ_PLAN_REGS);
	if (ret)
		return ret;

	mt7601u_rmw(dev, MT_TX_ALC_CFG_0, 0x3f3f, dev->ee->chan_pwr[chan_idx] & 0x3f);

	ret = mt7601u_write_reg_pairs(dev, MT_MCU_MEMMAP_BBP, bbp_settings, ARRAY_SIZE(bbp_settings));
	if (ret)
		return ret;
...
int mt7601u_phy_set_channel(struct mt7601u_dev *dev, struct cfg80211_chan_def *chandef)
{
	...
	ieee80211_queue_delayed_work(dev->hw, &dev->cal_work, MT_CALIBRATE_INTERVAL);
	if (dev->freq_cal.enabled)
		ieee80211_queue_delayed_work(dev->hw, &dev->freq_cal.work, MT_FREQ_CAL_INIT_DELAY);
	return 0;
}
...
static void mt7601u_rxdc_cal(struct mt7601u_dev *dev)
{
	...
	ret = mt7601u_write_reg_pairs(dev, MT_MCU_MEMMAP_BBP, intro, ARRAY_SIZE(intro));
	if (ret)
	...
	ret = mt7601u_write_reg_pairs(dev, MT_MCU_MEMMAP_BBP, outro, ARRAY_SIZE(outro));
	if (ret)
...
static void mt7601u_set_initial_tssi(struct mt7601u_dev *dev, s16 tssi_db, s16 tssi_hvga_db)
{
	...
	mt76_rmw(dev, MT_TX_ALC_CFG_1, MT_TX_ALC_CFG_1_TEMP_COMP, int_to_s6(init_offset) & MT_TX_ALC_CFG_1_TEMP_COMP);
}
...
static struct mt7601u_tssi_params mt7601u_tssi_params_get(struct mt7601u_dev *dev)
{
	...
	p.trgt_power += mt7601u_phy_rf_pa_mode_val(dev, pkt_type & 0x03, tx_rate);
...
{
	struct mt7601u_dev *dev = container_of(work, struct mt7601u_dev, cal_work.work);
...
__mt7601u_phy_freq_cal(struct mt7601u_dev *dev, s8 last_offset, u8 phy_mode)
{
	...
	return dev->freq_cal.adjusting ? MT_FREQ_CAL_ADJ_INTERVAL : MT_FREQ_CAL_CHECK_INTERVAL;
}

static void mt7601u_phy_freq_cal(struct work_struct *work)
{
	struct mt7601u_dev *dev = container_of(work, struct mt7601u_dev, freq_cal.work.work);
	s8 last_offset;
	u8 phy_mode;
	unsigned long delay;
...
int mt7601u_bbp_set_bw(struct mt7601u_dev *dev, int bw)
{
	...
	mt76_poll(dev, MT_MAC_STATUS, MT_MAC_STATUS_TX | MT_MAC_STATUS_RX, 0, 500000);
...
int mt7601u_phy_init(struct mt7601u_dev *dev)
{
	...
	if (ret)
		return ret;
	ret = mt7601u_write_reg_pairs(dev, 0, rf_central, ARRAY_SIZE(rf_central));
	if (ret)
		return ret;
	ret = mt7601u_write_reg_pairs(dev, 0, rf_channel, ARRAY_SIZE(rf_channel));
	if (ret)
		return ret;
