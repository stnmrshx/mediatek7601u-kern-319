...
{
	mt7601u_wr(dev, MT_MAC_SYS_CTRL, (MT_MAC_SYS_CTRL_RESET_CSR | MT_MAC_SYS_CTRL_RESET_BBP));
...
static int mt7601u_init_bbp(struct mt7601u_dev *dev)
{
	...
	ret = mt7601u_write_reg_pairs(dev, MT_MCU_MEMMAP_BBP, bbp_common_vals, ARRAY_SIZE(bbp_common_vals));
	if (ret)
		return ret;

	return mt7601u_write_reg_pairs(dev, MT_MCU_MEMMAP_BBP, bbp_chip_vals, ARRAY_SIZE(bbp_chip_vals));
}
...
{
	int ret;
	ret = mt7601u_write_reg_pairs(dev, MT_MCU_MEMMAP_WLAN, mac_common_vals, ARRAY_SIZE(mac_common_vals));
	if (ret)
		return ret;
	ret = mt7601u_write_reg_pairs(dev, MT_MCU_MEMMAP_WLAN, mac_chip_vals, ARRAY_SIZE(mac_chip_vals));
	if (ret)
		return ret;
...
static int mt7601u_init_wcid_mem(struct mt7601u_dev *dev)
{
	...
	ret = mt7601u_burst_write_regs(dev, MT_WCID_ADDR_BASE, vals, N_WCIDS * 2);
	kfree(vals);
	return ret;
}
...
{
	u32 vals[4] = {};
	return mt7601u_burst_write_regs(dev, MT_SKEY_MODE_BASE_0, vals, ARRAY_SIZE(vals));
}
...
static int mt7601u_init_wcid_attr_mem(struct mt7601u_dev *dev)
{
	...
	ret = mt7601u_burst_write_regs(dev, MT_WCID_ATTR_BASE, vals, N_WCIDS * 2);
	kfree(vals);
...
int mt7601u_mac_start(struct mt7601u_dev *dev)
{
	...
	if (!mt76_poll(dev, MT_WPDMA_GLO_CFG, MT_WPDMA_GLO_CFG_TX_DMA_BUSY | MT_WPDMA_GLO_CFG_RX_DMA_BUSY, 0, 200000))
		return -ETIMEDOUT;
	...
	mt7601u_wr(dev, MT_MAC_SYS_CTRL, MT_MAC_SYS_CTRL_ENABLE_TX | MT_MAC_SYS_CTRL_ENABLE_RX);

	if (!mt76_poll(dev, MT_WPDMA_GLO_CFG, MT_WPDMA_GLO_CFG_TX_DMA_BUSY | MT_WPDMA_GLO_CFG_RX_DMA_BUSY, 0, 50))
		return -ETIMEDOUT;
...
static void mt7601u_mac_stop_hw(struct mt7601u_dev *dev)
{
	...
	mt76_clear(dev, MT_MAC_SYS_CTRL, MT_MAC_SYS_CTRL_ENABLE_RX | MT_MAC_SYS_CTRL_ENABLE_TX);
...
int mt7601u_init_hardware(struct mt7601u_dev *dev)
{
	...
	mt7601u_wr(dev, MT_TXOP_CTRL_CFG, MT76_SET(MT_TXOP_TRUN_EN, 0x3f) | MT76_SET(MT_TXOP_EXT_CCA_DLY, 0x58));
...
{
	dev->sband_2g = devm_kzalloc(dev->dev, sizeof(*dev->sband_2g), GFP_KERNEL);
...
int mt7601u_register_device(struct mt7601u_dev *dev)
{
	...
	dev->wcid_mask[0] |= 1;
	dev->mon_wcid = devm_kmalloc(dev->dev, sizeof(*dev->mon_wcid), GFP_KERNEL);
	if (!dev->mon_wcid)
		return -ENOMEM;
