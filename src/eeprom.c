...
static int mt7601u_efuse_physical_size_check(struct mt7601u_dev *dev)
{
	...
	for (i = 0; i < map_reads; i++) {
		ret = mt7601u_efuse_read(dev, MT_EE_USAGE_MAP_START + i * 16, data + i * 16, MT_EE_PHYSICAL_READ);
		if (ret)
			return ret;
	}
	...
	if (MT_EFUSE_USAGE_MAP_SIZE - cnt_free < 5) {
		dev_err(dev->dev, "Error: needs default EEPROM file and this driver doesn't support it!\n");
		return -EINVAL;
	}
...
static void mt7601u_set_chip_cap(struct mt7601u_dev *dev, u8 *eeprom)
{
	...
	dev->ee->tssi_enabled = mt7601u_has_tssi(dev, eeprom) && !(nic_conf1 & MT_EE_NIC_CONF_1_TEMP_TX_ALC);
...
static int mt7601u_set_macaddr(struct mt7601u_dev *dev, const u8 *eeprom)
{
...
int mt7601u_eeprom_init(struct mt7601u_dev *dev)
{
	...
	if (eeprom[MT_EE_VERSION_EE] > MT7601U_EE_MAX_VER)
		dev_warn(dev->dev, "Warning: unsupported EEPROM version %02hhx\n", eeprom[MT_EE_VERSION_EE]);
